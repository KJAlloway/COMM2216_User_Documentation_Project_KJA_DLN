...
:   A tag in Java code that begins with `@`, used to pass information to
    the compiler or IDE. Common examples include `@Override`, `@author`,
    and `@param`.
...
:   The blinking cursor in the editor that marks your current position
    ...
    allowing you to edit several locations at once.
...
<span id="checkstyle-configuration">**Checkstyle configuration**</span>
:   An XML file that defines the exact set of style rules Checkstyle
    enforces. In CST courses, your instructor provides
    `COMP-2522-Checkstyle.xml` — the same file loaded into the
    CheckStyle-IDEA plugin and run against your submissions at grading time.
...
:   An automated analysis rule that IntelliJ IDEA applies to your code
    ...
    without requiring you to compile or run your program.

<span id="code-style">**Code style**</span>
:   A set of formatting and structural rules that define how code is
...
:   A background process IntelliJ IDEA runs when opening a project. It
    scans and catalogues all files so features like Search Everywhere and
    Go to Declaration work correctly. You can see it happening via a
    progress bar in the bottom-right corner of the IDE.
...
:   A modern, statically typed programming language designed by JetBrains
    and officially preferred for Android development. It runs on the Java
    Virtual Machine (JVM) and is fully interoperable with Java, allowing
    developers to write more concise, safe, and readable code.

## L

<span id="lint">**Lint / linting**</span>
:   An informal term for running static analysis tools — like Checkstyle
    or IntelliJ inspections — to catch code style and quality issues
    automatically.
...
:   A named, saved collection of IntelliJ IDEA inspection settings that
    can be exported as an XML file, shared, and imported into any IDE
    installation. Switching profiles changes which rules are active and
    at what severity. The CST inspection profile provided with this guide
    is pre-configured to match COMP 2522 style requirements.

<span id="project-tool-window">**Project tool window**</span>
:   The panel on the left side of the IntelliJ IDEA interface that
    displays your project's file and folder structure.

## R

<span id="refactoring">**Refactoring**</span>
:   Restructuring existing code to improve its design or readability
    without changing what the code actually does. Common operations
...
:   Whether a setting applies to a single project only, or to every
    project you open. Settings stored at the IDE level apply everywhere;
    settings stored at the project level apply only to that project.
...
:   The process of examining source code without running it, to identify
...
:   A class that another class inherits from. When class `B` extends
    ...
    fields and methods that the child class can use automatically. In
    COMP 2522, your `toString()` and `equals()` methods check whether a
    superclass exists and include its data in the output.
...
:   An import statement that uses `*` to import all classes from a
    ...
    are prohibited in COMP 2522 because they make it unclear which
    classes your code actually depends on.
